...
| Route every current iOS Standard entry to PlayerKit | **NO-GO** | The VOD gate passes, the offline `AVURLAsset` candidate passes its runtime/airplane-mode matrix, and catalog live plus TV transport routes are integrated and tested. |
...
| Automated library regression | N/A as app monolith. | Full PlayerKit `swift test` executes 337 tests with 5 environment skips and 0 failures; exact markers, quality policy/remapping, eight-rate menu/backend behavior, QoE, strings/privacy, recovery, WebVTT, host teardown/presentation policy, async PiP restoration and local-asset lifecycle have focused tests. | **Good code evidence, not app proof.** Skips need explicit disposition. | Keep suite green; document every skip; add smallest iTV host tests for business seams. | D |
| iTV integration compile | Standard app is existing build target. | Current `itv-new` generic Debug candidate compiles and links for both arm64 and x86_64 after the VOD, PiP-restoration and offline-routing slices. | **Compile proof only.** Simulator stayed shut down. | Clean-checkout build plus booted runtime smoke. | D |
...
| Product metadata | Title, description, poster, Cast/external URL, last position, episode index. | Exact typed skip segments and bounded thumbnail-VTT input added to `PlayerItem`; a narrow host presentation policy controls speed, quality and ended-overlay visibility without app business models; an optional host-provided `AVURLAsset` preserves local input through the AV backend. | Runtime thumbnail/ad/trailer/local-asset proof and explicit live/timeline policy. |
| System integrations | PiP, AirPlay, Google Cast, background audio, Now Playing, remote commands, capture shield. | iOS 14-compatible UI fallbacks and additional localized/accessibility copy; async PiP restoration completion lets the iTV host restore its actual presentation. | Real-device PiP/restoration proof and correct live Cast type. |
...
| iTV adapter | Basic New player, close/share/history, simple URL fetch/episode switching, basic paywall and metadata mapping. | Local candidate pin; stale package-reference cleanup; exact resume/metadata/markers/thumbnails; robust entitlement and async episode navigation; callback ownership; host-managed pause/exit teardown; late-response rejection; single exit-progress write lifetime; actual guarded PiP host restoration; PiP/background/Now Playing options; selected track/rate history; pause/end/fatal persistence; watch analytics/QoE bridge; deterministic app-owned IMA gate; identity-safe signed-URL refresh; stale-safe host quality discovery; app-owned report/timestamp menu; typed existing-series-catalog selection; strict trailer control/end/progress policy; complete candidate EN/RU/UZ `PlayerStrings` injection; both offline entries behind the selector with local-only validation and Core Data progress. | Real ad/expiry/quality/thumbnail/report/share/catalog/trailer proof, native-language approval, offline airplane-mode/PiP device proof, live/TV routes and broader runtime/device proof. |
...
| --- | --- | --- | --- |
| Full PlayerKit `swift test --no-parallel` | 337 executed, 5 environment skips, 0 failed after the PiP-restoration and local-asset slices. | Library logic in the active host environment; prior marker/quality/rate/WebVTT/policy/copy contracts plus async PiP completion and host-retained local-asset lifecycle remain green. | iOS frameworks, real media, iTV business flows, skipped environments or devices. |
